...
#### -----------------------   DATABASE CPNTAINER  --------------------------------
eval "SQL_CNTNR_IP=\$$SQL_CNTNR_NAME"
if [ "$SQL_CNTNR_IP" != "" ] ; then
	DATABASE_URL="mysql+pymysql://${MYSQL_USER}:${MYSQL_PASSWORD}@${SQL_CNTNR_IP}/${MYSQL_DATABASE}"
	echo "SQL_CNTNR_IP=${SQL_CNTNR_IP}" >>.env-ip
	echo "DATABASE_URL=${DATABASE_URL}" >>.env-ip
	echo && echo "SQL_CNTNR_IP=${SQL_CNTNR_IP}"
